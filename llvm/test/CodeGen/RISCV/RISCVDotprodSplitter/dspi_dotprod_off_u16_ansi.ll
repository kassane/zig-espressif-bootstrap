...
define dso_local range(i32 0, 458756) i32 @dspi_dotprod_off_u16_ansi(ptr noundef readonly captures(none) %in_image, ptr noundef readonly captures(none) %filter, ptr noundef writeonly captures(none) %out_value, i32 noundef %count_x, i32 noundef %count_y, i32 noundef %shift, i16 noundef zeroext %offset) local_unnamed_addr #0 {
; CHECK-LABEL: define dso_local range(i32 0, 458756) i32 @dspi_dotprod_off_u16_ansi(
; CHECK-SAME: ptr noundef readonly captures(none) [[IN_IMAGE:%.*]], ptr noundef readonly captures(none) [[FILTER:%.*]], ptr noundef writeonly captures(none) [[OUT_VALUE:%.*]], i32 noundef [[COUNT_X:%.*]], i32 noundef [[COUNT_Y:%.*]], i32 noundef [[SHIFT:%.*]], i16 noundef zeroext [[OFFSET:%.*]]) local_unnamed_addr #[[ATTR0:[0-9]+]] {
; CHECK-NEXT:  [[ENTRY:.*]]:
...
for.body28.us:                                    ; preds = %for.cond25.preheader.us, %for.body28.us
  ...
  %add.us = add nuw nsw i32 %conv34.us, %conv35
  %mul36.us = mul nuw nsw i32 %add.us, %conv.us
  %conv37.us = zext nneg i32 %mul36.us to i64
